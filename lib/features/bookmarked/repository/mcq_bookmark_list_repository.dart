...
class McqBookmarkListRepository {
  McqBookmarkListRepository({DioClient? dioClient})
      : _dioClient = dioClient ?? DioClient();

  final DioClient _dioClient;

  Future<ApiResponse<McqBookmarkListResponse>> getMcqBookmarkList() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == true) {
          final model = McqBookmarkListResponse.fromJson(
            data as Map<String, dynamic>,
          );
          return ApiResponse.success(data: model);
        }
        return ApiResponse.error(
          errorMsg: data['message']?.toString() ?? 'Unable to load bookmarks.',
        );
      }

      return ApiResponse.error(
        errorMsg: 'Unable to load bookmarks.',
      );
