...
class McqBookmarkDeleteRepository {
  McqBookmarkDeleteRepository({DioClient? dioClient})
      : _dioClient = dioClient ?? DioClient();

  final DioClient _dioClient;

  Future<ApiResponse<McqBookmarkDeleteResponse>> deleteMcqBookmark({
    ...
    try {
      ...
      final response = await _dioClient.getDio().post(
        APIConstants.deleteMcqBookmarks,
        queryParameters: {
          'stu_id': user.stuId,
          'mcq_id': mcqId,
        },
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == true) {
          final model = McqBookmarkDeleteResponse.fromJson(
            data as Map<String, dynamic>,
          );
          return ApiResponse.success(data: model);
        }
        return ApiResponse.error(
          errorMsg: data['message']?.toString() ?? 'Unable to delete bookmark.',
        );
      }

      return ApiResponse.error(
        errorMsg: 'Unable to delete bookmark.',
      );
