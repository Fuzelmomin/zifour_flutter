...
class McqFeedbackRepository {
  McqFeedbackRepository({DioClient? dioClient})
      : _dioClient = dioClient ?? DioClient();

  final DioClient _dioClient;

  Future<ApiResponse<McqFeedbackResponse>> addMcqFeedback({
    required String mcqId,
    required String mcqType,
    required String mcqFdbTitle,
    required String mcqFdbDescription,
  }) async {
    try {
      ...
      final response = await _dioClient.getDio().post(
        APIConstants.addMcqFeedback,
        queryParameters: {
          'stu_id': user.stuId,
          'mcq_id': mcqId,
          'mcq_typ_id': mcqType,
          'mcq_fdb_title': mcqFdbTitle,
          'mcq_fdb_description': mcqFdbDescription,
        },
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == true) {
          final model = McqFeedbackResponse.fromJson(
            data as Map<String, dynamic>,
          );
          return ApiResponse.success(data: model);
        }
        return ApiResponse.error(
          errorMsg: data['message']?.toString() ?? 'Unable to add feedback.',
        );
      }

      return ApiResponse.error(
        errorMsg: 'Unable to add feedback.',
      );
