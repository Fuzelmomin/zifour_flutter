...
class McqBookmarkRepository {
  McqBookmarkRepository({DioClient? dioClient})
  ...
  Future<ApiResponse<McqBookmarkResponse>> addMcqBookmark({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == true) {
          final model = McqBookmarkResponse.fromJson(
            data as Map<String, dynamic>,
          );
          return ApiResponse.success(data: model);
        }
        return ApiResponse.error(
          errorMsg: data['message']?.toString() ?? 'Unable to add bookmark.',
        );
      }
