...
class McqNotesListRepository {
  McqNotesListRepository({DioClient? dioClient})
      : _dioClient = dioClient ?? DioClient();

  final DioClient _dioClient;

  Future<ApiResponse<McqNotesListResponse>> getMcqNotesList() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == true) {
          final model = McqNotesListResponse.fromJson(
            data as Map<String, dynamic>,
          );
          return ApiResponse.success(data: model);
        }
        return ApiResponse.error(
          errorMsg: data['message']?.toString() ?? 'Unable to load notes.',
        );
      }

      return ApiResponse.error(
        errorMsg: 'Unable to load notes.',
      );
