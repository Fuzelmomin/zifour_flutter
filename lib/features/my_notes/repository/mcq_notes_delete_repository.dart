...
class McqNotesDeleteRepository {
  McqNotesDeleteRepository({DioClient? dioClient})
      : _dioClient = dioClient ?? DioClient();

  final DioClient _dioClient;

  Future<ApiResponse<McqNotesDeleteResponse>> deleteMcqNote({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == true) {
          final model = McqNotesDeleteResponse.fromJson(
            data as Map<String, dynamic>,
          );
          return ApiResponse.success(data: model);
        }
        return ApiResponse.error(
          errorMsg: data['message']?.toString() ?? 'Unable to delete note.',
        );
      }

      return ApiResponse.error(
        errorMsg: 'Unable to delete note.',
      );
